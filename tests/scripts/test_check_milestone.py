...
def test_check_for_milestone(mock_github_client, mock_github_graph_ql_client):
    ...
    mock_query_result.json.return_value = {
        'data': {
            'repository': {
                'pullRequest': {
                    'closingIssuesReferences': {'nodes': [{'number': 1234}, {'number': 1235}]}
                }
            }
        }
    }
    ...
    mock_github_client.return_value.get.assert_has_calls([
        call(github_org='sdv-dev', repo='sdv', endpoint='issues/1234'),
        call().json(),
        call(github_org='sdv-dev', repo='sdv', endpoint='issues/1235'),
        call().json(),
    ])
...
@patch('scripts.check_milestone.GithubGraphQLClient')
@patch('scripts.check_milestone.GithubClient')
def test_check_for_milestone_different_milestones(mock_github_client, mock_github_graph_ql_client):
    # Setup
    # Setup
    mock_query_result = Mock()
    mock_query_result.json.return_value = {
    ...
    # Run and assert
    expected_message = 'This PR resolves issues with multiple different milestones.'
    with pytest.raises(Exception, match=expected_message):
        check_for_milestone(1222)
